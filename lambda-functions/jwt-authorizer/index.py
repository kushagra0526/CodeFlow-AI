...
def handler(event, context):
    ...
    try:
        ...
        # Generate IAM policy - use wildcard to allow all API endpoints
        # This is necessary because API Gateway caches the policy for 5 minutes
        # and we don't want to restrict access to specific endpoints
        method_arn = event.get('methodArn', '')
        # Extract the API Gateway ARN prefix (everything before the HTTP method)
        # Format: arn:aws:execute-api:region:account:api-id/stage/METHOD/resource
        arn_parts = method_arn.split('/')
        if len(arn_parts) >= 2:
            # Create wildcard ARN: arn:aws:execute-api:region:account:api-id/stage/*
            api_gateway_arn = f"{arn_parts[0]}/{arn_parts[1]}/*"
        else:
                ...
                'Statement': [
                    {
                        ...
                        'Effect': 'Allow',
                        'Resource': api_gateway_arn  # Allow all endpoints in this API/stage
                    }
                ]
            },
            'context': {
